...
<div class="admin-form-card ai-writer-card">
  ...
  <div class="ai-writer-row"><select id="aiProvider" class="form-control"><option value="gemini">Gemini</option><option value="deepseek">DeepSeek</option><option value="openai">OpenAI</option><option value="openrouter">OpenRouter</option></select><input id="aiTopic" class="form-control" placeholder="Topik artikel, misalnya: manfaat murajaah harian"><select id="aiTone" class="form-control"><option value="informatif dan hangat">Informatif</option><option value="inspiratif dan menyentuh">Inspiratif</option><option value="formal dan edukatif">Formal</option></select><button type="button" id="aiGenerate" class="btn-sm btn-sm-primary">Generate</button></div><p id="aiStatus" class="ai-status" role="status"></p>
</div>
...
<script>
(function(){var b=document.getElementById('aiGenerate');if(!b)return;b.addEventListener('click',function(){var topic=document.getElementById('aiTopic').value.trim(),status=document.getElementById('aiStatus');if(!topic){status.textContent='Topik wajib diisi.';return;}b.disabled=true;status.textContent='AI sedang menulis...';var body=new FormData();body.append('csrf_token','<?= generateCsrfToken() ?>');body.append('provider',document.getElementById('aiProvider').value);body.append('topic',topic);body.append('tone',document.getElementById('aiTone').value);fetch('<?= BASE_URL ?>/api/generate-article.php',{method:'POST',body:body,credentials:'same-origin'}).then(function(r){return r.json().then(function(j){if(!r.ok)throw new Error(j.error||'Gagal');return j;});}).then(function(j){document.getElementById('judul').value=j.article.judul;document.getElementById('ringkasan').value=j.article.ringkasan;var isi=document.getElementById('isi');isi.value=j.article.isi;isi.dispatchEvent(new Event('input'));status.textContent='Draft AI siap. Silakan tinjau dan edit.';}).catch(function(e){status.textContent=e.message;}).finally(function(){b.disabled=false;});});})();
</script>
